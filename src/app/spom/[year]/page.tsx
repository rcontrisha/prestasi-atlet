...
const fetchSpomData = async (
  ...
  const res = await fetch(`${supabaseUrl}/rest/v1/rpc/get_spom_data_by_year`, {
    method: "POST",
    headers,
    body: JSON.stringify({
...
const fetchSpomCount = async (
  ...
  const res = await fetch(`${supabaseUrl}/rest/v1/rpc/get_spom_count_by_year`, {
    method: "POST",
    headers,
    body: JSON.stringify({
      year: Number(year),
...
const fetchCabors = async (year: string): Promise<string[]> => {
  const res = await fetch(`${supabaseUrl}/rest/v1/rpc/get_spom_sports_by_year`, {
    method: "POST",
    headers,
    body: JSON.stringify({ year_value: Number(year) }),
  });
...
export default function SpomYearView() {
  ...
  const loadData = async () => {
    ...
    try {
      const [items, count] = await Promise.all([
        fetchSpomData(year, search, filterCabor, currentPage, perPage),
        fetchSpomCount(year, search, filterCabor),
      ]);
  ...
  useEffect(() => {
    fetchCabors(year).then(setCabors).catch(console.error);
  }, [year]);

  useEffect(() => {
    loadData();
  }, [search, filterCabor, currentPage, perPage, year]);

  return (
    <div className="min-h-screen bg-white">
      <NavbarComponent title={`SPOM ${year}`} />
              ...
              <table className="w-full text-sm text-left text-gray-600 rounded-lg">
                <thead className="text-xs uppercase bg-gray-200 text-gray-700">
                  <tr>
                    ...
                    <th className="px-4 py-2">Nomor Kejuaraan</th>
                  ...
                  {data.length > 0 ? (
                    data.map((item, index) => (
                      <tr key={item.id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-4 py-2">{item.nomor_kejuaraan}</td>
